...
def get_possible_schedules(courses):
    combos = itertools.product(*courses_ex)
    possible_schedules = []
    for combo in combos:
...
def get_nth_schedule(n, course_data, target_start, target_end):
    return score_schedules(get_possible_schedules(course_data),target_start,target_end)[n-1]

def get_scored_schedules(course_data, target_start, target_end):
    return score_schedules(get_possible_schedules(course_data),target_start,target_end)

print_schedule_with_scores(get_nth_schedule(10, courses_ex, 1200, 1700)) # 1 is best, 0 is worst
